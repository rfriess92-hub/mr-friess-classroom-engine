...
- preserve every output requested in the brief's `required_outputs` field
- be suitable for `schema:check`, `route:plan`, `render:package`, and `qa:bundle`

`qa:render` remains a single-artifact drill-down tool, not the package-level generation gate.
...
11. Preserve every output requested in required_outputs.
12. Map ordinary teacher phrasing to canonical output types: PowerPoint/PPT/slide deck -> slides, student packet -> task_sheet, marking guide/answer guide -> answer_key.
13. Do not emit schema-only blocked output types as classroom-ready outputs.
...
- `required_outputs` is authoritative and must drive `bundle.declared_outputs` and package outputs.
...
- If `answer_key` is present, keep it teacher-facing.
- If `final_response_sheet` or `exit_ticket` is the evidence location, final evidence must live there.
- If the brief asks for a generic rubric, prefer `rubric_sheet` unless the task deliberately targets future schema-only `rubric` work.
...
4. Verify required-output preservation.
5. Run:
   - `schema:check`
   - `route:plan`
   - `render:package`
...
- perform full semantic classroom QA across rendered artifacts
