...
function buildTicket(pkg, section) {
  const title = section.title ?? 'Exit Ticket'
  const prompt = section.prompt ?? ''
  const lineCount = section.n_lines ?? 4
...
<div class="ticket">
  ...
  <div class="ticket-topic">${escapeHtml(title)}</div>
  <div class="ticket-prompt-label">Before you leave</div>
  <div class="ticket-prompt">${formatPrompt(prompt)}</div>
  <div class="ticket-lines">
    ${Array.from({ length: lineCount }, () => '<div class="ticket-line"></div>').join('\n')}
  </div>
</div>`
}

function buildCutLine() {
  return '<div class="cut-line"><span class="cut-icon">Scissor line</span></div>'
}
