...
function buildShell(pkg, section, title, subtitle, body, fontFaceCSS, designCSS) {
  ...
  return `<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><title>${escapeHtml(title)}</title><style>${fontFaceCSS}\n${designCSS}\n${css()}</style></head><body class="lvt-body"><main class="lvt-page"><div class="lvt-name-date"><span><strong>Name:</strong><i></i></span><span><strong>Date:</strong><i></i></span></div><header class="lvt-title-block"><h1>${escapeHtml(title)}</h1><p>${escapeHtml(subtitle)}</p>${meta ? `<div class="lvt-meta">${escapeHtml(meta)}</div>` : ''}</header>${body}<footer class="lvt-footer">Vocabulary tools for noticing, breaking apart, and using words accurately.</footer></main></body></html>`
}
...
.lvt-page { position:relative; width:8.5in; height:11in; padding:.29in .50in .34in; overflow:hidden; }
.lvt-page::before { content:""; position:absolute; top:0; left:0; right:0; height:4pt; background:#7B4FBE; }
.lvt-name-date { display:flex; justify-content:space-between; gap:22pt; margin:0 0 13pt; font-size:9pt; font-weight:700; }
...
.lvt-title-block { text-align:center; margin:0 0 12pt; }
.lvt-title-block h1 { color:#161C2D; font-size:19pt; line-height:1.08; margin:0 0 3pt; font-weight:700; }
.lvt-title-block p { color:#78879C; font-size:10pt; line-height:1.25; margin:0; }
.lvt-meta { color:#78879C; font-size:7.8pt; letter-spacing:.16em; text-transform:uppercase; margin-top:4pt; }
.lvt-panel { border:1pt solid #D8DDE8; border-radius:4pt; background:#fff; overflow:hidden; }
.lvt-panel-head { min-height:25pt; display:flex; align-items:center; gap:7pt; padding:6pt 9pt; background:#F5F6F8; border-bottom:1pt solid #D8DDE8; color:#161C2D; font-weight:700; font-size:10.3pt; }
...
.lvt-panel-body { padding:8pt 10pt; }
.lvt-grid { display:grid; gap:8pt; }
.lvt-two { grid-template-columns:1fr 1fr; }
.lvt-three { grid-template-columns:repeat(3,1fr); }
.lvt-directions { margin-bottom:8pt; }
.lvt-directions .lvt-panel-body { font-size:9pt; }
.lvt-line { height:.22in; border-bottom:1pt solid #CBD2DB; }
.lvt-line.short { height:.16in; }
.lvt-fill-line { display:inline-block; flex:1; height:10pt; border-bottom:1pt solid #CBD2DB; }
.lvt-check-row { display:grid; grid-template-columns:12pt 1fr; gap:7pt; align-items:start; font-size:8.9pt; line-height:1.25; margin-bottom:5pt; }
.lvt-checkbox { width:10pt; height:10pt; border:1.15pt solid #1E2738; margin-top:1pt; }
.lvt-footer { position:absolute; left:.50in; right:.50in; bottom:.18in; border-top:1pt solid #D8DDE8; color:#78879C; font-size:7.8pt; padding-top:5pt; text-align:center; }
.lvt-frayer-grid { display:grid; grid-template-columns:1fr 1fr; gap:8pt; margin-top:8pt; }
.lvt-frayer-card { min-height:2.25in; }
.lvt-frayer-card .lvt-panel-body { min-height:1.92in; }
.lvt-word-center { display:flex; align-items:center; justify-content:center; min-height:.72in; border:2pt solid #7B4FBE; border-radius:5pt; background:#EDE5F8; color:#161C2D; font-size:18pt; font-weight:700; margin:8pt 0; text-align:center; }
.lvt-card-grid { display:grid; grid-template-columns:1fr 1fr; gap:8pt; margin-top:8pt; }
.lvt-vocab-card { min-height:2.2in; border:1pt solid #D8DDE8; border-radius:5pt; overflow:hidden; page-break-inside:avoid; }
.lvt-vocab-title { background:#EDE5F8; border-bottom:1pt solid #D8DDE8; color:#161C2D; font-size:13pt; font-weight:700; padding:7pt 9pt; }
.lvt-vocab-body { padding:8pt 9pt; display:grid; gap:5pt; }
.lvt-card-label { color:#7B4FBE; font-weight:700; font-size:8pt; text-transform:uppercase; letter-spacing:.08em; }
.lvt-prefix-grid { display:grid; grid-template-columns:1fr 1fr 1.15fr; gap:8pt; margin-top:8pt; }
.lvt-word-row { border:1pt solid #D8DDE8; border-radius:5pt; overflow:hidden; margin-bottom:7pt; }
...
.lvt-word-row-head span, .lvt-word-row-body div { padding:6pt 7pt; border-right:1pt solid #D8DDE8; }
.lvt-word-row-head span:last-child, .lvt-word-row-body div:last-child { border-right:0; }
.lvt-word-row-body { display:grid; grid-template-columns:1.1fr 1fr 1fr; min-height:.5in; }
.lvt-so-what { margin-top:8pt; }
`
}
...
function buildFrayer(pkg, section, fontFaceCSS, designCSS) {
  ...
  const body = `${directions(section, 'Use each box to define, describe, and test your understanding of the focus word.')}<div class="lvt-word-center">${escapeHtml(focus)}</div><div class="lvt-frayer-grid">${panel('Definition in my own words', lines(6), 'DEF')}${panel('Characteristics / parts', lines(6), 'PART')}${panel('Examples', lines(6), 'EX')}${panel('Non-examples', lines(6), 'NO')}</div><div class="lvt-so-what">${panel('Use the word accurately', `<div>Write one clear sentence that proves you understand the word.</div>${lines(2)}`, 'USE')}</div>${section.success_criteria ? `<div style="margin-top:8pt;">${panel('Before you move on', checkboxList(section.success_criteria), 'CHK')}</div>` : ''}`
...
function buildVocabularyCards(pkg, section, fontFaceCSS, designCSS) {
  ...
  const body = `${directions(section, 'Cut apart or fold these cards. Add a meaning, word-part clue, and original sentence for each word.')}<div class="lvt-card-grid">${cards}</div>${section.success_criteria ? `<div style="margin-top:8pt;">${panel('Check your cards', checkboxList(section.success_criteria), 'CHK')}</div>` : ''}`
