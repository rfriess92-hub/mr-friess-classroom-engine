...
function numberedSection(number, title, prompt, lineCount = 2, iconLabel = null) {
  const iconHtml = iconLabel ? icon(iconLabel) : `<span class="cws-number">${escapeHtml(String(number))}</span>`
  return `
    <section class="cws-response-box">
      <div class="cws-response-head">${iconHtml}<div><strong>${escapeHtml(title)}</strong><span>${escapeHtml(prompt)}</span></div></div>
      ${lines(lineCount)}
    </section>`
}

function checkboxList(items, lineFallback = false) {
  ...
  if (lineFallback && normalized.length === 0) {
    return rows.join('')
  }
  return rows.join('')
}
...
function buildShell(pkg, section, title, subtitle, body, fontFaceCSS, designCSS) {
  const subject = text(pkg?.subject, 'Class')
  const grade = text(pkg?.grade, '')
  const topic = text(pkg?.topic, '')
  const gradeLabel = grade ? `Grade ${escapeHtml(grade)}` : ''
  const meta = [subject, gradeLabel, topic].filter(Boolean).join(' · ')
...
.cws-name-date span:first-child {
  max-width: 3.0in;
}

.cws-name-date span:last-child {
  max-width: 2.15in;
}

.cws-name-date i {
  display: inline-block;
  border-bottom: 1.7pt solid #111;
  flex: 1;
  height: 0;
}
...
.cws-grid.left-sidebar { grid-template-columns: 1fr 2.1in; align-items: stretch; }
.cws-grid.sidebar-left { grid-template-columns: 5.2in 2.1in; align-items: stretch; }
.cws-grid.bottom-pair { grid-template-columns: 1.5fr 1fr; }
...
.cws-line {
  border-bottom: 1.05pt solid #111;
  height: 0.22in;
}
...
.cws-organizer-grid .cws-box-body {
  min-height: 1.48in;
}
...
.cws-venn-left { left: 0.1in; }
.cws-venn-right { right: 0.1in; }
.cws-venn-both { position: absolute; left: 50%; top: 0.43in; transform: translateX(-50%); text-align: center; font-size: 8.2pt; width: 0.75in; }
...
.cws-web-center { left: 50%; top: 50%; transform: translate(-50%, -50%); width: 1.05in; height: 0.78in; }
.cws-web-node { width: 0.72in; height: 0.55in; }
.cws-web-node.n1 { left: 0.12in; top: 0.1in; }
.cws-web-node.n2 { right: 0.12in; top: 0.1in; }
.cws-web-node.n3 { left: 0.12in; bottom: 0.1in; }
.cws-web-node.n4 { right: 0.12in; bottom: 0.1in; }

.cws-flow {
  display: grid;
  grid-template-columns: 1fr 0.18in 1fr 0.18in 1fr;
  gap: 4pt;
  align-items: center;
}
...
.cws-question-box.q1 { left:0.05in; top:0.08in; }
.cws-question-box.q2 { right:0.05in; top:0.08in; }
.cws-question-box.q3 { left:0.05in; bottom:0.08in; }
.cws-question-box.q4 { right:0.05in; bottom:0.08in; }
...
.cws-segmented-word span {
  border-right: 1pt dotted #111;
}
...
.cws-handwriting .cws-line {
  border-bottom-style: solid;
  box-shadow: 0 -7pt 0 -6pt #666 inset;
  height: 0.22in;
}

.cws-tight .cws-title-block h1 { font-size: 23pt; }
.cws-tight .cws-title-block { margin-bottom: 6pt; }
...
function buildTemplateSystem(pkg, section, fontFaceCSS, designCSS) {
  const rules = [
    ['1. Clear Header', 'Use one strong title and a simple subtitle.', '□'],
    ['2. Simple Sections', 'Break the page into clear boxes with bold labels.', '▦'],
    ['3. Writable Space', 'Leave enough lines and blank areas for student thinking.', '✎'],
    ['4. Student Supports', 'Add vocabulary, checklist, or examples when needed.', '★'],
    ['5. Consistent Icons', 'Use small icons only to cue the section purpose.', '☆'],
    ['6. Print-Friendly', 'Keep everything high-contrast and uncluttered.', '⎙'],
  ]

  const body = `
    <div class="cws-grid cws-template-rules">
      ${rules.map(([title, bodyText, iconLabel]) => `
        <section class="cws-rule-box">
          ${icon(iconLabel)}
          <div><div class="cws-rule-title">${escapeHtml(title)}</div><div class="cws-rule-body">${escapeHtml(bodyText)}</div></div>
        </section>`).join('')}
    </div>
    ${sectionBox('Core Layout Blocks', `
      <div class="cws-core-grid">
        ${sectionBox('Learning Target', '[Learning Target]', { icon: '◎' })}
        ${sectionBox('Directions', '[Directions]', { icon: '➜' })}
        ${sectionBox('Reading / Task', '[Insert short text / prompt here]', { icon: '□', className: 'cws-core-tall' })}
        ${sectionBox('Answer Lines', `[Student response area]${lines(5)}`, { icon: '✎', className: 'cws-core-wide' })}
        ${sectionBox('Checklist / Reflection', `[Success criteria / reflection]${checkboxList([], true)}`, { icon: '✓' })}
      </div>
    `)}
    <div class="cws-footer-banner">Built for English 8, Careers, Literacy Intervention, Science, and Social Studies</div>
  `
  return buildShell(pkg, section, templateTitle(section, 'Worksheet Template System'), templateSubtitle(section, 'Reusable classroom layout for future handouts'), body, fontFaceCSS, designCSS)
}
...
function buildReadingResponse(pkg, section, fontFaceCSS, designCSS) {
  ...
  const body = `
    <div class="cws-grid two">
      ${headerBox('Learning Target', text(section.learning_target, '[I can ...]'), '◎')}
      ${headerBox('Directions', text(section.directions, '[Read / think / respond]'), '✓')}
    </div>
    ${sectionBox('Text / Prompt', `<div>${escapeHtml(text(section.prompt, '[Insert passage, quotation, image prompt, or question here.]'))}</div>`, { icon: '□' })}
    <div class="cws-grid left-sidebar" style="margin-top:8pt;">
      <div class="cws-stack">
        ...
        ${numberedSection(3, 'Explain Your Thinking', '[How does your evidence connect?]', 3, '?')}
        ${numberedSection(4, 'Personal / Text Connection', '[What does this make you think about?]', 2, '∞')}
      </div>
      <aside class="cws-stack">
        ${sectionBox('Vocabulary Support', wordDefinitionRows(vocab), { icon: '□' })}
        ${sectionBox('Success Criteria', checkboxList(success), { icon: '✓' })}
      </aside>
    </div>
    ${sectionBox('Reflection', `[What did you do well? What do you want to improve next time?]${lines(2)}`, { icon: '…' })}
  `
...
function buildCer(pkg, section, fontFaceCSS, designCSS) {
  ...
  const body = `
    <div class="cws-grid three">
      ${headerBox('Question / Topic', text(section.question, '[What are you answering?]'), '?')}
      ${headerBox('Learning Target', text(section.learning_target, '[I can make a clear claim and support it.]'), '◎')}
      ${headerBox('Directions', text(section.directions, '[Use evidence and explain your reasoning.]'), '➜')}
    </div>
    <div class="cws-grid sidebar-left" style="margin-top:8pt;">
      <div class="cws-stack">
        ${numberedSection(1, 'Claim', '[What is your answer or conclusion?]', 4, '★')}
        ${sectionBox('2 Evidence', `
        ...
        `, { icon: '⌕' })}
        ${numberedSection(3, 'Reasoning', '[How does your evidence support your claim?]', 5, '!')}
      </div>
      <aside class="cws-stack">
        ${sectionBox('Sentence Starters', starters.map((starter) => `<div class="cws-vocab-row">* ${escapeHtml(starter)}</div>`).join(''), { icon: '“' })}
        ${sectionBox('Checklist', checkboxList(checklist), { icon: '✓' })}
      </aside>
    </div>
    ${sectionBox('Extension / Counterclaim', `[What could someone else say? How would you respond?]${lines(3)}`, { icon: '=' })}
  `
...
function buildVocabularyMorphology(pkg, section, fontFaceCSS, designCSS) {
  ...
  const body = `
    <div class="cws-grid two">
      ${headerBox('Word / Term', targetWord, '□')}
      ${headerBox('Directions', text(section.directions, '[Use the boxes to break down and understand the word.]'), '➜')}
    </div>
    ...
    ${sectionBox('Morphology Breakdown', `
      <div class="cws-morph-grid">
        ...
        <div><strong>Meaning of Parts</strong>${lines(4)}</div>
        <div><strong>What the Whole Word Means</strong>${lines(4)}</div>
      </div>
    `, { icon: '▣' })}
    <div class="cws-grid two">
      ${sectionBox('Use It in a Sentence', lines(4), { icon: '✎' })}
      ${sectionBox('Sketch / Memory Clue', '<div style="height:1.1in;"></div>', { icon: '□' })}
    </div>
    <div class="cws-grid two">
      ${sectionBox('Synonyms / Related Words', lines(3), { icon: '∞' })}
      ${sectionBox('Reflection', `[How will I remember this word?]${lines(2)}`, { icon: '✓' })}
    </div>
  `
...
function buildCareersDecision(pkg, section, fontFaceCSS, designCSS) {
  const body = `
    <div class="cws-grid three">
      ${headerBox('Scenario / Task', text(section.scenario, '[Insert situation or decision]'), '◎')}
      ${headerBox('Goal', text(section.goal, '[What am I trying to do?]'), '⚑')}
      ${headerBox('Directions', text(section.directions, '[Think, sort, choose, explain.]'), '➜')}
    </div>
    ${sectionBox('Option Planner', `
      <div class="cws-grid cws-option-grid">
        ${[1,2,3].map((num) => `
          <div class="cws-option-card">
            <h3>${num === 1 ? '1' : num === 2 ? '2' : '3'} Option ${num}</h3>
            <div class="cws-field-label">What is it?</div>${lines(2)}
            <div class="cws-field-label">Benefits</div>${lines(2)}
            <div class="cws-field-label">Challenges</div>${lines(2)}
            <div class="cws-field-label">What might happen next?</div>${lines(2)}
          </div>`).join('')}
      </div>
    `)}
    ${sectionBox('What Matters Most?', `
      <table class="cws-matrix">
        ...
        <tbody>
          <tr><th>Time</th><td></td><td></td><td></td></tr>
          <tr><th>Importance</th><td></td><td></td><td></td></tr>
          <tr><th>Effort</th><td></td><td></td><td></td></tr>
          <tr><th>Impact</th><td></td><td></td><td></td></tr>
        </tbody>
      </table>
    `)}
    <div class="cws-grid bottom-pair">
      ${sectionBox('My Choice', `[Which option makes the most sense right now? Why?]${lines(4)}`, { icon: '●' })}
      ${sectionBox('Decision Checklist', checkboxList(['I considered more than one option.', 'I looked at the benefits and challenges.', 'I thought about what matters most.', 'I explained my reason clearly.', 'I am ready to take the next step.']), { icon: '✓' })}
    </div>
    ${sectionBox('Next Step', `[What will I do first?]${lines(2)}`, { icon: '➜' })}
  `
...
function buildLiteracyWordStudy(pkg, section, fontFaceCSS, designCSS) {
  const body = `
    <div class="cws-grid three">
      ${headerBox('Skill Focus', text(section.skill_focus, '[ch / sh / syllable type / suffix / etc.]'), '◎')}
      ${headerBox('Learning Target', text(section.learning_target, '[I can ...]'), '▱')}
      ${headerBox('Directions', text(section.directions, '[Read, sort, build, write.]'), '➜')}
    </div>
    ${sectionBox('Sound / Pattern Practice', `
      <div class="cws-grid two">
        ${headerBox('Target Pattern', text(section.target_pattern, '[Insert pattern]'), '~')}
        ${headerBox('Key Sound / Rule', text(section.key_rule, '[Insert rule]'), '!')}
      </div>
      <div class="cws-grid cws-sound-boxes" style="margin-top:7pt;">
        ${Array.from({ length: 6 }, (_, i) => `<div class="cws-mini-write"><span class="cws-number">${i + 1}</span>${lines(2, 'dotted')}</div>`).join('')}
      </div>
    `)}
    <div class="cws-grid two">
      ${sectionBox('Sort It', `<div class="cws-grid cws-sort-grid">${['[Category A]','[Category B]','[Category C]'].map((label) => `<div class="cws-mini-write"><strong>${escapeHtml(label)}</strong>${lines(5)}</div>`).join('')}</div>`)}
      ${sectionBox('Tap / Map / Mark', `<div class="cws-grid cws-map-grid">${Array.from({ length: 4 }, () => '<div class="cws-map-box"></div>').join('')}</div>`)}
    </div>
    ${sectionBox('Read and Write', `
      <div class="cws-grid cws-readwrite">
        <div><strong>Read These Words</strong>${Array.from({ length: 6 }, (_, i) => `<div style="display:grid;grid-template-columns:18pt 1fr;gap:4pt;align-items:center;"><span>${i + 1}.</span><div class="cws-line"></div></div>`).join('')}</div>
        <div class="cws-handwriting"><strong>Write These Words</strong>${Array.from({ length: 6 }, (_, i) => `<div style="display:grid;grid-template-columns:18pt 1fr;gap:4pt;align-items:center;"><span>${i + 1}.</span><div class="cws-line"></div></div>`).join('')}</div>
      </div>
    `)}
    <div class="cws-grid three">
      ${sectionBox('Sentence Practice', lines(2), { icon: '…' })}
      ${sectionBox('Challenge Word', '<div style="height:.55in;"></div>', { icon: '★' })}
      ${sectionBox('Self-Check', checkboxList(['I read the words.', 'I sorted or mapped correctly.', 'I spelled the words.', 'I checked my work.']), { icon: '✓' })}
    </div>
  `
...
function buildGraphicOrganizerPack(pkg, section, fontFaceCSS, designCSS) {
  const body = `
    <div class="cws-grid cws-organizer-grid">
      ${sectionBox('1 Compare & Contrast', `<div class="cws-venn"><div class="cws-venn-circle cws-venn-left">[Idea A]${lines(4)}</div><div class="cws-venn-circle cws-venn-right">[Idea B]${lines(4)}</div><div class="cws-venn-both">[Both]${lines(3)}</div></div>`, { icon: '∞' })}
      ${sectionBox('2 Main Idea + Details', `<div class="cws-web"><div class="cws-web-center">[Main Idea]${lines(2)}</div><div class="cws-web-node n1">${lines(3)}</div><div class="cws-web-node n2">${lines(3)}</div><div class="cws-web-node n3">${lines(3)}</div><div class="cws-web-node n4">${lines(3)}</div></div>`, { icon: '⌘' })}
      ${sectionBox('3 Cause and Effect', `<div class="cws-flow"><div class="cws-flow-card">[Cause]${lines(3)}</div><div class="cws-arrow">→</div><div class="cws-flow-card">[Effect]${lines(3)}</div><div class="cws-arrow">→</div><div class="cws-flow-card">[Next Effect]${lines(3)}</div></div>`, { icon: '⚡' })}
      ${sectionBox('4 Timeline', `<div class="cws-timeline"><div class="cws-timeline-line"></div><div class="cws-timeline-events">${Array.from({ length: 5 }, () => `<div class="cws-event"><div class="cws-event-dot"></div><div class="cws-event-box">[Event]${lines(2)}</div></div>`).join('')}</div></div>`, { icon: '◷' })}
      ${sectionBox('5 Problem / Solution', `<div class="cws-problem"><div class="cws-problem-top">[Problem]${lines(3)}</div><div class="cws-problem-pair"><div class="cws-problem-bottom">[Possible Solution]${lines(3)}</div><div class="cws-problem-bottom">[Best Solution]${lines(3)}</div></div></div>`, { icon: '▣' })}
      ${sectionBox('6 Question Web', `<div class="cws-question-web"><div class="cws-question-center">[Big Question]</div><div class="cws-question-box q1"></div><div class="cws-question-box q2"></div><div class="cws-question-box q3"></div><div class="cws-question-box q4"></div></div>`, { icon: '?' })}
    </div>
    <div class="cws-footer-banner">Use these layouts to organize ideas before writing, speaking, or discussing.</div>
  `
  return buildShell(pkg, section, templateTitle(section, 'Graphic Organizer Template Pack'), templateSubtitle(section, 'Reusable thinking tools for English, science, socials, and Careers'), body, fontFaceCSS, designCSS)
}

function buildExitReflection(pkg, section, fontFaceCSS, designCSS) {
  const body = `
    <div class="cws-grid two">
      ${headerBox('Today’s Topic', text(section.topic_prompt ?? section.topic, '[Insert topic]'), '◎')}
      ${headerBox('Learning Target', text(section.learning_target, '[I can ...]'), '□')}
    </div>
    <div class="cws-grid left-sidebar" style="margin-top:8pt;">
      <div class="cws-stack">
      ...
      <aside class="cws-stack">
        ${sectionBox('Self-Assessment', ['I need help', 'I’m getting there', 'I understand', 'I could teach this'].map((label) => `<label class="cws-radio-row"><span class="cws-radio"></span><span>${escapeHtml(label)}</span></label>`).join(''))}
        ${sectionBox('Teacher Note', '<div style="height:2.4in;"></div>')}
      </aside>
    </div>
    ${sectionBox('Reflection', `[How did you participate or use your time today?]${lines(2)}`, { icon: '…' })}
  `
...
function buildGenericGraphicOrganizer(pkg, section, fontFaceCSS, designCSS) {
  ...
  const body = `
    ${section.prompt ? sectionBox('Directions', escapeHtml(section.prompt), { icon: '➜' }) : ''}
    ...
    `)}
    ${section.success_criteria ? sectionBox('Before you move on', checkboxList(section.success_criteria), { icon: '✓' }) : ''}
  `
