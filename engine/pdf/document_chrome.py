...
def fit_header_text(text: str, *, string_width, font_name: str = 'Helvetica', font_size: float = 8.5, max_width: float) -> str:
    collapsed = _collapse_whitespace(text)
    if not collapsed:
        return ''
    if string_width(collapsed, font_name, font_size) <= max_width:
        return collapsed

    ellipsis = '…'
    working = collapsed
    while working and string_width(f'{working}{ellipsis}', font_name, font_size) > max_width:
        working = working[:-1].rstrip()

    if not working:
        return ellipsis
    return f'{working}{ellipsis}'
...
def chrome_callback(packet: dict | None, output_type: str | None, section: dict | None = None):
    ...
    def _draw(canvas, doc):
        ...
        available_width = max(48, right_x - left_x - 24 - stringWidth(left_text, 'Helvetica', 8.5))
        fitted_right_text = fit_header_text(
            right_text,
        ...
        canvas.drawRightString(right_x, header_y, fitted_right_text)
