...
def _task_profile_from_hints(task):
    ...
    if hints.get('lines') is not None:
        p['lines'] = max(1, int(hints['lines']))
    if hints.get('row_height') is not None:
        p['row_height'] = max(12, int(hints['row_height']))
    return p
...
def task_profile(task):
    ...
    if label == 'Part A' and 'strongest reason' in lowered:
        p = dict(TASK_PROFILES['day2_reason_keep'])
    elif label == 'Part B' and ('evidence' in lowered or 'explanation' in lowered):
        p = dict(TASK_PROFILES['day2_evidence_revision'])
    elif label == 'Part C' and 'this evidence matters because' in lowered:
        p = dict(TASK_PROFILES['day2_stronger_explanation'])
    elif label == 'Part A' and 'state your opinion clearly' in lowered:
        p = dict(TASK_PROFILES['day1_issue_opinion'])
    elif label == 'Part B' and 'two reasons' in lowered:
        p = dict(TASK_PROFILES['day1_reason_list'])
    elif label == 'Part C' and 'detail from the evidence set' in lowered:
        p = dict(TASK_PROFILES['day1_evidence_pick'])
    elif label == 'Part D' and 'how does this evidence support your reason' in lowered:
        p = dict(TASK_PROFILES['day1_evidence_explain'])
    elif label == 'Part E' and 'weak spot' in lowered:
        p = dict(TASK_PROFILES['day1_revision_focus'])
    else:
        p = dict(TASK_PROFILES['generic'])
        p['heading'] = label or p['heading']
        p['lines'] = max(1, int(task.get('lines', p['lines'])))
...
def _choice_pairs(task):
    ...
    for index, item in enumerate(_task_render_hints(task).get('choice_pairs', []) if isinstance(_task_render_hints(task).get('choice_pairs', []), list) else []):
        ...
        label = _string_value(item.get('label')) or f'Item {index + 1}'
        if left and right:
...
def _matching_columns(task):
    ...
    if not isinstance(value, dict):
        return None
    left_items = normalize_string_list(value.get('left_items', []))
    right_items = normalize_string_list(value.get('right_items', []))
...
def task_response_pattern(task):
    ...
    activity = _string_value(task.get('activity_type'))
    lines = max(1, int(task.get('lines', 1) or 1))
    prompt = _string_value(task.get('prompt')).lower()
...
def build_paired_choice_block(styles, task, pairs, show_help=True, spacer_after=6):
    ...
    for pair in pairs:
        rows.append([Paragraph(pair['label'], styles['MicroX']), _selector_option_cell(styles, pair['left'], 198), _selector_option_cell(styles, pair['right'], 198)])
...
def build_matching_block(styles, task, matching, show_help=True, spacer_after=6):
    ...
    rows = [[Paragraph(matching['left_label'], styles['PromptLabelX']), Paragraph('Letter', styles['PromptLabelX']), Paragraph(matching['right_label'], styles['PromptLabelX'])]]
...
def build_record_fields_block(base, styles, task, fields, show_help=True, spacer_after=6):
    profile = task_profile(task)
    cards = [_record_field_card(base, styles, field['label']) for field in fields]
    rows = []
...
def build_calculation_workspace_block(base, styles, task, show_help=True, spacer_after=6):
    profile = task_profile(task)
    hints = _task_render_hints(task)
    answer_label = _string_value(hints.get('answer_label')) or 'Final answer'
    workspace_rows = max(2, int(hints.get('workspace_rows', 6) or 6))
...
def build_compact_checkpoint_block(base, styles, task, rendered_lines=None, show_help=True, spacer_after=6):
    profile = task_profile(task)
    line_total = max(1, min(3, int(rendered_lines if rendered_lines is not None else profile['lines'])))
    response_card = _framed_flow_card([Paragraph('Brief response', styles['ResponseLabelX']), Spacer(1, 2), base.response_line_table(line_total, row_height=14)], width=540, bg_color=CARD_BG, border_color=BORDER, padding=6)
...
def integrated_task_box(base, styles, profile, line_total=None, show_help=True, spacer_after=6):
    lines = max(2, int(line_total if line_total is not None else profile['lines']))
    response = base.response_line_table(lines, row_height=profile['row_height'])
    prompt_inner = [Paragraph(profile['heading'], styles['SectionHeadX']), Paragraph(profile['instruction'], styles['BodyText'])]
    if show_help and profile['help']:
        prompt_inner += [Spacer(1, 2), Paragraph(profile['help'], styles['InlineHelpX'])]
    ...
    response_card = Table([[response_inner]], colWidths=[540])
    response_card.setStyle(TableStyle([('BACKGROUND', (0, 0), (-1, -1), CARD_BG), ('BOX', (0, 0), (-1, -1), 0.65, BORDER), ('TOPPADDING', (0, 0), (-1, -1), 6), ('BOTTOMPADDING', (0, 0), (-1, -1), 6), ('LEFTPADDING', (0, 0), (-1, -1), 8), ('RIGHTPADDING', (0, 0), (-1, -1), 8), ('VALIGN', (0, 0), (-1, -1), 'TOP')]))
    return KeepTogether([prompt_card, Spacer(1, 3), response_card, Spacer(1, spacer_after)])


def build_task_block(base, styles, task, compact=False, spacing_scale: float = 1.0, rendered_lines=None, show_help: bool = True, **_kwargs):
    pattern = task_response_pattern(task)
    spacer_after = max(2, int(round((5 if compact else 6) * spacing_scale)))
...
def _footer_table(story, left, right=None, left_bg=None, right_bg=None, border_color=None):
    lb = left_bg or SUPPORT_BG
    rb = right_bg or SUCCESS_BG
    bc = border_color or SUPPORT_BORDER
    if right is not None:
        left_card = Table([[left]], colWidths=[265])
        left_card.setStyle(TableStyle([('BACKGROUND', (0, 0), (-1, -1), lb), ('BOX', (0, 0), (-1, -1), 0.45, bc), ('TOPPADDING', (0, 0), (-1, -1), 5), ('BOTTOMPADDING', (0, 0), (-1, -1), 5), ('LEFTPADDING', (0, 0), (-1, -1), 7), ('RIGHTPADDING', (0, 0), (-1, -1), 7), ('VALIGN', (0, 0), (-1, -1), 'TOP')]))
        right_card = Table([[right]], colWidths=[265])
        right_card.setStyle(TableStyle([('BACKGROUND', (0, 0), (-1, -1), rb), ('BOX', (0, 0), (-1, -1), 0.45, SUCCESS_BORDER), ('TOPPADDING', (0, 0), (-1, -1), 5), ('BOTTOMPADDING', (0, 0), (-1, -1), 5), ('LEFTPADDING', (0, 0), (-1, -1), 7), ('RIGHTPADDING', (0, 0), (-1, -1), 7), ('VALIGN', (0, 0), (-1, -1), 'TOP')]))
        ...
        footer = Table([[left]], colWidths=[540])
        footer.setStyle(TableStyle([('BACKGROUND', (0, 0), (-1, -1), lb), ('BOX', (0, 0), (-1, -1), 0.45, bc), ('TOPPADDING', (0, 0), (-1, -1), 5), ('BOTTOMPADDING', (0, 0), (-1, -1), 5), ('LEFTPADDING', (0, 0), (-1, -1), 7), ('RIGHTPADDING', (0, 0), (-1, -1), 7), ('VALIGN', (0, 0), (-1, -1), 'TOP')]))
    story.append(footer)
...
def _grammar_layout(grammar, tasks):
    intent = grammar.get('render_intent', 'exploratory_planning')
    density = grammar.get('density', 'medium')
    evidence = grammar.get('evidence_role', 'planning_only')
    band = grammar.get('length_band', 'standard')
    multi = density == 'heavy' and len(tasks) >= 5
    dense_single = not multi and len(tasks) >= 4
    compact = multi or dense_single or density == 'heavy' or intent in ('revision_strengthen', 'checkpoint_prep')
...
def _display_title(base, intent, raw):
    neutral = base.neutralize_student_task_title(raw) if raw else 'Task Sheet'
    suffix = _INTENT_TITLE_SUFFIX.get(intent, '')
...
def _estimate_structured_task_height(task, compact):
    ...
    if pattern == 'matching':
        matching = _matching_columns(task)
        row_count = max(len(matching['left_items']), len(matching['right_items'])) if matching else 3
...
def _estimate_task_block_height(base, task, compact, rendered_lines=None, show_help=True):
    ...
    heading = base.estimate_wrapped_lines(profile['heading'], 70)
    instruction = base.estimate_wrapped_lines(profile['instruction'], 88 if compact else 82)
    help_lines = base.estimate_wrapped_lines(profile['help'], 90) if show_help and profile['help'] else 0
...
def _split_tasks_for_multi_page(base, layout, tasks):
    if len(tasks) <= 1:
        return tasks, []
    line_counts = _scaled_lines(layout['length_band'], len(tasks), 3)
    budget = 455 if layout['checkpoint_close'] else 495
    current = 0
    split_index = 0
    for index, task in enumerate(tasks[:-1]):
        show_help = _help_visible(layout, index, len(tasks), 0)
        task_height = _estimate_task_block_height(base, task, True, line_counts[index], show_help)
        remaining = 0
        for later_index, later_task in enumerate(tasks[index + 1:], start=index + 1):
            later_show_help = _help_visible(layout, later_index - (index + 1), len(tasks[index + 1:]), 1)
            remaining += _estimate_task_block_height(base, later_task, True, line_counts[later_index], later_show_help)
        if index >= 1 and current + task_height > budget:
            break
        if index >= 1 and current + task_height > budget - 18 and remaining >= 220:
            break
        current += task_height
        split_index = index + 1
...
def _add_support_success_footer(styles, story, section, max_support_items=2, max_success_items=3):
    raw = normalize_string_list(section.get('embedded_supports', []))[:max_support_items]
    support = [clean_support_text(i) for i in raw]
    success = normalize_string_list(section.get('success_criteria', []))[:max_success_items]
    if support and success:
        _footer_table(story, compact_list_cell(styles, _support_footer_title(raw), support, box_color=SUPPORT_ACCENT), compact_list_cell(styles, 'Quick check-in', success, box_color=SUCCESS_ACCENT))
        return
    if support:
        _footer_table(story, compact_list_cell(styles, _support_footer_title(raw), support, box_color=SUPPORT_ACCENT), right_bg=None)
        return
    if success:
        _footer_table(story, compact_list_cell(styles, 'Quick check-in', success, box_color=SUCCESS_ACCENT), left_bg=SUCCESS_BG, border_color=SUCCESS_BORDER)
...
def render_task_sheet(base, styles_bundle, packet, section, out_path):
    ...
    instructions = _instruction_lines(section, layout)
    ...
    if instructions:
        _add_focus_rail(styles, story, instructions, compact=layout['compact'])
    if layout['multi_page']:
        page1, page2 = _split_tasks_for_multi_page(base, layout, tasks)
        page1_lines = _scaled_lines(layout['length_band'], len(page1), 3)
        for index, (task, lines) in enumerate(zip(page1, page1_lines)):
            story.append(base.build_task_block(styles, task, compact=True, spacing_scale=0.5, rendered_lines=lines, show_help=_help_visible(layout, index, len(page1), 0)))
        story.append(PageBreak())
        base.title_bar(story, styles, base.packet_heading(packet))
        suffix = '— Checkpoint' if layout['checkpoint_close'] else '— Continued'
        story.append(Paragraph(f"{base.neutralize_student_task_title(section.get('title', 'Task Sheet'))} {suffix}", styles['SheetTitleX']))
        story.append(Spacer(1, 3))
        page2_lines = _scaled_lines(layout['length_band'], len(page2), 3)
        for index, (task, lines) in enumerate(zip(page2, page2_lines)):
            story.append(base.build_task_block(styles, task, compact=True, spacing_scale=0.56, rendered_lines=lines, show_help=_help_visible(layout, index, len(page2), 1)))
        if layout['checkpoint_close']:
            base.add_day1_page2_footer(story, styles, section)
        else:
            _add_support_success_footer(styles, story, section, 2, 2)
    elif layout['compact']:
        base_lines = 3 if layout['dense_single_page'] else 4
        line_counts = _scaled_lines(layout['length_band'], len(tasks), base_lines)
        spacing_scale = 0.62 if layout['dense_single_page'] else 0.72
        support_items = 1 if layout['dense_single_page'] else 2
        for index, (task, lines) in enumerate(zip(tasks, line_counts)):
            story.append(base.build_task_block(styles, task, compact=True, spacing_scale=spacing_scale, rendered_lines=lines, show_help=_help_visible(layout, index, len(tasks), 0)))
        if layout['checkpoint_close']:
            base.add_day2_footer(story, styles, section)
        else:
            _add_support_success_footer(styles, story, section, support_items, 2)
    else:
...
def final_support_text(section):
    ...
    parts = [clean_support_text(i) for i in ps.get('frame_strip', []) if str(i).strip()]
    reminder = clean_support_text(ps.get('reminder_box', '')).strip()
...
def final_writing_zone_block(base, styles, story, section):
    ...
    zone.setStyle(TableStyle([('BACKGROUND', (0, 0), (-1, -1), CARD_BG), ('BOX', (0, 0), (-1, -1), 0.75, BORDER), ('TOPPADDING', (0, 0), (-1, -1), 8), ('BOTTOMPADDING', (0, 0), (-1, -1), 8), ('LEFTPADDING', (0, 0), (-1, -1), 10), ('RIGHTPADDING', (0, 0), (-1, -1), 10), ('VALIGN', (0, 0), (-1, -1), 'TOP')]))
...
def final_closing_band(styles, story, section):
    ...
    right = compact_list_cell(styles, 'Quick check-in', ['Ready to hand in', 'Mostly there', 'Need help with one part'], box_color=SUCCESS_ACCENT)
    footer = Table([[left, right]], colWidths=[265, 265])
    footer.setStyle(TableStyle([('BACKGROUND', (0, 0), (-1, -1), FINAL_FOOTER_BG), ('BOX', (0, 0), (-1, -1), 0.55, FINAL_FOOTER_BORDER), ('INNERGRID', (0, 0), (-1, -1), 0.35, FINAL_FOOTER_GRID), ('TOPPADDING', (0, 0), (-1, -1), 6), ('BOTTOMPADDING', (0, 0), (-1, -1), 6), ('LEFTPADDING', (0, 0), (-1, -1), 8), ('RIGHTPADDING', (0, 0), (-1, -1), 8), ('VALIGN', (0, 0), (-1, -1), 'TOP')]))
    story.append(footer)
...
def render_final_response_sheet(base, styles_bundle, packet, section, out_path):
    ...
    if assessment_weight == 'high':
        story += [Spacer(1, 5), Paragraph('This is your final assessed response. Write your best, most complete answer.', styles['PurposeLineX'])]
    else:
        story.append(Spacer(1, 3))
    ...
    if assessment_weight == 'high' and length_band in ('standard', 'extended'):
        section = {**section, 'response_lines': max(14, int(section.get('response_lines', 12)) + 2)}
