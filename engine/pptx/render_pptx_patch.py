...
def render_stat_discussion(slide, content: dict, theme: dict) -> None:
    ...
    add_card(slide, 5.65, 1.80, 6.55, 4.60, content.get("prompt_title", "Discuss"), accent, theme["tints"]["secondary"])
    add_card_bullets(slide, 5.98, 2.36, 5.95, 3.60, [str(x) for x in content.get("prompts", [])], font_size=18)


def render_two_column(slide, columns: list[dict], theme: dict) -> None:
...
def render_three_rows(slide, rows: list[dict], theme: dict) -> None:
    y = 1.60
...
def render_retrieval(slide, content: dict, theme: dict) -> None:
    add_textbox(slide, 0.95, 1.55, 11.4, 0.42, "Use the prompts below to build your response.", font_size=17, color=SLATE, align=PP_ALIGN.CENTER)
    y = 2.00
    fallback_accents = [theme["primary"], theme["secondary"], theme["tertiary"]]
    for i, pr in enumerate(content.get("prompts", [])[:3]):
        accent = hex_to_rgb(pr.get("accent"), fallback_accents[i % len(fallback_accents)])
        add_card(slide, 1.0, y, 11.0, 1.10, f"Prompt {i+1}", accent, LIGHT)
        add_textbox(slide, 1.32, y + 0.40, 10.2, 0.28, pr.get("text", ""), font_size=17, color=NAVY)
        y += 1.28


def render_summary_rows(slide, rows: list[dict], theme: dict) -> None:
    y = 1.80
    fallback_accents = [theme["primary"], theme["secondary"], theme["tertiary"]]
    for i, row in enumerate(rows[:3]):
        accent = hex_to_rgb(row.get("accent"), fallback_accents[i % len(fallback_accents)])
...
def render_single_card(slide, content: dict, theme: dict) -> None:
    ...
    add_card(slide, 1.0, 2.00, 11.0, content.get("card_height", 2.2), "Goal plan", accent, LIGHT)
    lines = []
...
def render_reflect(slide, goals: list[str], accent: RGBColor) -> None:
    add_textbox(slide, 0.95, 1.55, 11.4, 0.40, "Check yourself against today’s goals.", font_size=18, color=SLATE, align=PP_ALIGN.CENTER)
    y = 2.10
    for i, goal in enumerate(goals[:3], start=1):
        add_card(slide, 1.05, y, 10.2, 0.95, f"Goal {i}", accent, LIGHT)
        add_textbox(slide, 1.38, y + 0.33, 8.8, 0.26, goal, font_size=16, color=NAVY)
...
def render_slide(prs: Presentation, slide, packet: dict, slide_spec: dict, theme: dict) -> None:
    ...
    if layout == "hero":
        ...
        if layout == "stat_discussion":
            ...
            render_two_column(slide, content.get("columns", []), theme)
        elif layout == "three_rows":
            render_three_rows(slide, content.get("rows", []), theme)
            ...
            render_summary_rows(slide, content.get("rows", []), theme)
        elif layout == "single_card":
            render_single_card(slide, content, theme)
        elif layout == "reflect":
            render_reflect(slide, content.get("goals", []), accent)
        else:
