...
function buildTaskSheetBlocks(route, section) {
  ...
  for (const task of Array.isArray(section.tasks) ? section.tasks : []) {
    pushTextBlock(blocks, route, 'workflow_section', task.label || task.prompt, { priority: 'high', source_key: 'tasks.label' })
    blocks.push(
      makeBlock(route, 'response_area', {
        priority: 'high',
        source_key: 'tasks.response_area',
        estimated_lines: Math.max(2, Number(task.lines ?? 2)),
      }),
    )
  }
  pushListBlock(blocks, route, 'callout', section.embedded_supports, { priority: 'support', source_key: 'embedded_supports' })
  pushListBlock(blocks, route, 'checklist', section.success_criteria, { priority: 'normal', source_key: 'success_criteria' })
  return blocks
}
...
function buildWorksheetBlocks(route, section) {
  ...
  for (const question of Array.isArray(section.questions) ? section.questions : []) {
    pushTextBlock(blocks, route, 'instruction', question.q_text, { priority: 'high', source_key: 'questions.q_text' })
    blocks.push(
      makeBlock(route, 'response_area', {
        priority: 'high',
        source_key: 'questions.response_area',
        estimated_lines: Math.max(2, Number(question.n_lines ?? 2)),
      }),
    )
  }
  pushListBlock(blocks, route, 'checklist', section.self_check, { priority: 'normal', source_key: 'self_check' })
  pushListBlock(blocks, route, 'teacher_note', section.answer_key, { priority: 'support', source_key: 'answer_key', teacher_only: true, student_facing: false })
  return blocks
}
...
function buildFinalResponseBlocks(route, section) {
  ...
  pushListBlock(blocks, route, 'callout', section.planning_reminders, { priority: 'normal', source_key: 'planning_reminders' })
  if (isObject(section.paragraph_support)) {
    pushListBlock(blocks, route, 'callout', section.paragraph_support.frame_strip, { priority: 'support', source_key: 'paragraph_support.frame_strip' })
...
function buildGraphicOrganizerBlocks(route, section) {
  ...
  if (Array.isArray(section.columns) && section.columns.length > 0) {
    blocks.push(makeBlock(route, 'table', { priority: 'high', source_key: 'columns', estimated_lines: Math.max(3, Number(section.rows ?? 1) + 1) }))
  }
  pushListBlock(blocks, route, 'checklist', section.success_criteria, { priority: 'normal', source_key: 'success_criteria' })
  return blocks
}
...
function buildDiscussionPrepBlocks(route, section) {
  ...
  blocks.push(makeBlock(route, 'response_area', { priority: 'high', source_key: 'position_label', estimated_lines: 2 }))
  if (Number(section.evidence_count ?? 0) > 0) {
    for (let i = 0; i < Number(section.evidence_count); i += 1) {
      blocks.push(makeBlock(route, 'response_area', { priority: 'high', source_key: `evidence_count.${i + 1}`, estimated_lines: 2 }))
    }
  }
  if (section.include_question) {
    blocks.push(makeBlock(route, 'response_area', { priority: 'normal', source_key: 'include_question', estimated_lines: 2 }))
  }
  if (section.include_counterargument) {
    blocks.push(makeBlock(route, 'response_area', { priority: 'normal', source_key: 'include_counterargument', estimated_lines: 2 }))
  }
  pushListBlock(blocks, route, 'checklist', section.success_criteria, { priority: 'normal', source_key: 'success_criteria' })
  return blocks
}
...
function buildCheckpointSheetBlocks(route, section) {
  ...
  pushListBlock(blocks, route, 'checklist', section.look_fors, { priority: 'high', source_key: 'look_fors' })
  pushListBlock(blocks, route, 'teacher_note', section.conference_prompts, { priority: 'normal', source_key: 'conference_prompts', teacher_only: true, student_facing: false })
...
function buildSlidesBlocks(route, slides) {
  ...
  for (const slide of Array.isArray(slides) ? slides : []) {
    pushTextBlock(blocks, route, 'title', slide.title, { priority: 'high', source_key: 'slides.title' })
    if (isObject(slide.content)) {
      pushTextBlock(blocks, route, 'subtitle', slide.content.subtitle, { priority: 'high', source_key: 'slides.content.subtitle' })
      pushTextBlock(blocks, route, 'intro', slide.content.scenario ?? slide.content.task, { priority: 'high', source_key: 'slides.content.scenario' })
      pushListBlock(blocks, route, 'bullets', slide.content.prompts ?? slide.content.rows ?? slide.content.bullets, { priority: 'normal', source_key: 'slides.content.list' })
      pushTextBlock(blocks, route, 'caption', slide.content.caption, { priority: 'support', source_key: 'slides.content.caption' })
    }
    if (slide.image_intent || slide.content?.image) {
      blocks.push(makeBlock(route, 'image', { priority: 'normal', source_key: 'slides.image', estimated_lines: 6, allow_split: false }))
    }
  }
  return blocks
}
...
function buildTeacherGuideLikeBlocks(route, section) {
  ...
  for (const [key, type] of scalarKeys) {
    pushTextBlock(blocks, route, type, section[key], { priority: type === 'assessment_note' || type === 'exemplar' ? 'high' : 'normal', source_key: key, teacher_only: true, student_facing: false })
  }
  ...
  for (const [key, type] of listKeys) {
    pushListBlock(blocks, route, type, section[key], { priority: type === 'quick_tool' ? 'high' : 'normal', source_key: key, teacher_only: true, student_facing: false })
  }

  pushTableOrWorkflow(blocks, route, 'sequence', section.sequence, { priority: 'high', teacher_only: true, student_facing: false })
  pushTableOrWorkflow(blocks, route, 'timing', section.timing, { priority: 'high', teacher_only: true, student_facing: false })
  pushTableOrWorkflow(blocks, route, 'workflow', section.workflow, { priority: 'high', teacher_only: true, student_facing: false })

  if (Array.isArray(section.matching_bank) && section.matching_bank.length > 0) {
    blocks.push(makeBlock(route, 'quick_tool', { priority: 'high', source_key: 'matching_bank', estimated_lines: section.matching_bank.length * 2, teacher_only: true, student_facing: false }))
  }

  if (isObject(section.model) || Array.isArray(section.model)) {
    blocks.push(makeBlock(route, 'exemplar', { priority: 'high', source_key: 'model', estimated_lines: 6, teacher_only: true, student_facing: false }))
  }
...
function buildGenericDocBlocks(route, section) {
  ...
  for (const [key, value] of Object.entries(section)) {
    ...
    if (typeof value === 'string') {
      ...
      pushTextBlock(blocks, route, blockType, value, { priority: 'normal', source_key: key })
      continue
    }
    if (Array.isArray(value) && value.every((item) => typeof item === 'string')) {
      const blockType = /criteria|check|look_fors/.test(key) ? 'checklist' : 'bullets'
      pushListBlock(blocks, route, blockType, value, { priority: 'normal', source_key: key })
      continue
    }
    if (Array.isArray(value) && value.every((item) => isObject(item))) {
      pushTableOrWorkflow(blocks, route, key, value, { priority: 'normal', source_key: key })
    }
  }
  return blocks
}
