...
test('pbg_workplace_math10 fixture validates and routes cleanly', async () => {
  const { planPackageRoutes } = await import('../../engine/planner/output-router.mjs')
  const { loadJson } = await import('../../scripts/lib.mjs')
  const pkg = loadJson('fixtures/plan-build-grow/pbg_workplace_math10.json')
...
test('pbg_workplace_math10 smoke renders when python path is available', { timeout: 60000 }, (t) => {
  if (!pythonWithReportlabAvailable()) {
    t.skip('Python with reportlab is not available in this environment')
    return
  }

  const outDir = mkdtempSync(join(tmpdir(), 'classroom-engine-pbg-wm10-'))
  try {
    const result = spawnSync(
      process.execPath,
      ['scripts/render-package.mjs', '--fixture', 'pbg_workplace_math10', '--out', outDir, '--flat-out'],
