...
test('pbg_math8 fixture validates and routes cleanly', async () => {
  const { planPackageRoutes } = await import('../../engine/planner/output-router.mjs')
  const { loadJson } = await import('../../scripts/lib.mjs')
  const pkg = loadJson('fixtures/plan-build-grow/pbg_math8.json')
...
test('pbg_math8 smoke renders when python path is available', { timeout: 60000 }, (t) => {
  if (!pythonWithReportlabAvailable()) {
    t.skip('Python with reportlab is not available in this environment')
    return
  }

  const outDir = mkdtempSync(join(tmpdir(), 'classroom-engine-pbg-math8-'))
  try {
    const result = spawnSync(
      process.execPath,
      ['scripts/render-package.mjs', '--fixture', 'pbg_math8', '--out', outDir, '--flat-out'],
