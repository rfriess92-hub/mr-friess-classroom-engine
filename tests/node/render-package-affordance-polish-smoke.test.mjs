...
test('render-package smoke renders task-sheet affordance polish proof fixture when python path is available', { timeout: 45000 }, (t) => {
  if (!pythonWithReportlabAvailable()) {
    t.skip('Python with reportlab is not available in this environment')
    return
  }

  const outDir = mkdtempSync(join(tmpdir(), 'classroom-engine-affordance-polish-smoke-'))
  try {
