...
assert.equal(routes.length, 2, 'Expected separate assessment and teacher guide routes')
...
assert.ok(studentRoute, 'Missing student assessment route')
assert.ok(teacherRoute, 'Missing teacher marking route')
...
assert.equal(studentRoute.artifact_family, 'assessment_pack')
...
assert.equal(teacherRoute.artifact_family, 'marking_guide')
...
console.log('psychology-foundations-assessment-route ok: student assessment and teacher marking guide separated')
