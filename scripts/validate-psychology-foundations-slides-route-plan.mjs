...
assert.equal(routes.length, 1, 'Expected one slide route')
...
assert.equal(slideRoute.audience, 'student')
assert.equal(slideRoute.audience_bucket, 'student_facing')
assert.equal(slideRoute.renderer_key, 'render_slides')
assert.equal(slideRoute.renderer_family, 'pptx')
assert.equal(slideRoute.artifact_family, 'slide_deck')
...
assert.ok(blocks.length >= 30, 'Expected slide source to produce a substantial block set')
assert.equal(blocks.some((block) => block.teacher_only === true), false, 'Student slide blocks should not be teacher-only')
