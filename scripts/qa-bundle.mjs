...
function buildPatches() {
  return [
    ...
    },
    {
      rank: 3,
      type: 'content_issue',
      patch: 'Use package and assessment answer-leak QA findings to remove teacher-only answer or marking fields from student-facing artifacts.'
    }
  ]
}
...
const packageAnswerLeakQa = runPackageAnswerLeakQa(pkg)
...
if (packageAnswerLeakQa.blockers.length > 0) {
  blockers.push(...packageAnswerLeakQa.blockers)
  findings.push(...packageAnswerLeakQa.findings)
} else if (packageAnswerLeakQa.applies) {
  fastScore += 2
}
...
emit({
  ...
  package_answer_leak_qa: packageAnswerLeakQa,
