...
import { FIXTURE_MAP, argValue, fail, hasFlag, loadJson, printFixtureList, repoPath, resolvePackageTargets } from './lib.mjs'
...
const strictDir = hasFlag('--strict-dir')
const flatDir = hasFlag('--flat-dir')
...
if (targets.length === 0) {
  ...
  console.log('Usage: pnpm run qa:bundle -- (--package <path> | --fixture <key> | --fixture-pattern <glob>) --out output [--strict-dir] [--flat-dir] [--list-fixtures]')
  console.log('Default behavior reads artifacts from output/<package_id>/ to match package-scoped rendering.')
  process.exit(0)
}

function runBundleQa(target) {
  ...
  const currentFixtureKey = target.fixtureKey ?? '__current__'
  ...
  const crossPackageCollisions = flatDir && target.fixtureKey ? summarizeCollisions(target.fixtureKey, expectedFilesByFixture) : []
...
for (const target of targets) {
  if (targets.length > 1) {
    console.log(`\n=== Bundle QA fixture: ${target.label} (${target.path}) ===`)
  }
  if (runBundleQa(target)) hasFailure = true
}
